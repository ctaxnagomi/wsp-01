    ...
    <div className={`p-6 md:p-10 glass-dark rounded-[2.5rem] border border-white/10 ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          color="bg-neu-accent/20" 
          textColor="text-neu-accent"
        />
        <StatCard 
          label="Cinematic Movies" 
          value={stats.movies} 
          icon={<i className="fa-solid fa-film text-base" />} 
        ...
        <StatCard 
          ...
          value={stats.series} 
          icon={<i className="fa-solid fa-tv text-base" />} 
        ...
        <StatCard 
          ...
          value={stats.anime} 
          icon={<i className="fa-solid fa-wand-magic-sparkles text-base" />} 
...
const StatCard: React.FC<{ label: string, value: number, icon: React.ReactNode, color: string, textColor: string }> = ({ label, value, icon, color, textColor }) => (
  <div className="glass-light p-6 rounded-3xl border border-white/5 hover:border-white/20 transition-all group hover:translate-y-[-4px]">
    <div className="flex items-center justify-between mb-4">
      <div className={`w-10 h-10 rounded-2xl ${color} flex items-center justify-center ${textColor} shadow-xl`}>
        {icon}
      </div>
